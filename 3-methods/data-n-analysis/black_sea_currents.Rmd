...
* read in a shapefile (from GIS_Ecology_BG). This results in a Spdf object (= spatial polygons data frame).    
```{r background_map_shp}
## this needs library rgdal
# my_spdf <- rgdal::readOGR(dsn = here(), layer = "TM_WORLD_BORDERS_SIMPL-0.3") 
...
ggplot(dummy.currents, aes(x = lon, y = lat)) +
  geom_polygon(data = bg.coast, mapping = aes(x = long, y = lat, group = group), fill = "darkgrey", colour = "grey45") +
  geom_segment(aes(xend = lon + u * 0.25, yend = lat + v * 0.25),
arrow = arrow(angle = 15, length = unit(0.02, "inches"), type = "closed"), alpha = 0.3) + 
  ## zoom in on the coastline
  coord_fixed(xlim = c(27.3, 29),  ylim = c(42, 43), ratio = 1.3)
