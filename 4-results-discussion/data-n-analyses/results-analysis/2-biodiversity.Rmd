...
dplus.sim <- parSapply(core.clust, sim.list, function(x) replicate(999, simulate_Dplus(master.tax.list, tax.dist.sand, species = x)))

stopCluster(core.clust) ## ALWAYS stop cluster afterwards!
rm(core.clust)

## transform the matrix into a long tibble, and clean & arrange it a little
colnames(dplus.sim) <- sim.list

dplus.sim.long <- as_tibble(dplus.sim) %>% 
  gather(key = sp_number, value = Dplus_sim) %>%
  mutate(sp_number = as.numeric(sp_number)) %>%
  arrange(sp_number)

## calculate the 95% confidence intervals for each group (sp_number)
...
s.dplus.sand <- bind_cols(S = tax.distinctness.sand$Species, 
                          Dplus = tax.distinctness.sand$Dplus, 
                          station = zoo.abnd.sand$station)

ggplot(s.dplus.sand, aes(x = S, y = Dplus, color = station)) + 
...
```{r summarize_tax_distinctness_sand}
