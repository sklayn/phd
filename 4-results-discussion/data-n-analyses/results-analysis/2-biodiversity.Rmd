...
# generate the Dplus values for each simulated dataset 

### NB better write a function for a single simulaton, then run it the requried nb times (simulate_Dplus(999, species = x))

### Also do something like this: matrix(simulate_Dplus(999*29), nrow = 999, ncol = 29)

  for(n in 1:ncol(sim.tib)) {
    vec <- numeric(nrow(sim.tib))  
    for(i in 1:length(vec)) {
      y <- sample_n(master.tax.list, size = as.numeric(colnames(sim.tib)[n]), replace = F)
      # needed because taxondive is grumpy 
      y.df <- data.frame(matrix(1, nrow = 1, ncol = nrow(y)))
      names(y.df) <- row.names(y)
      # very much needed, because floods everything with complaints about sp lists not matching in length
      vec[i] <- suppressMessages(taxondive(y.df, tax.dist.sand)$Dplus)
      }
    sim.tib[, n] <- vec
    }
