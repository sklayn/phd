...
* some sediment samples were damaged, and sometimes not taken at all at several of the sand stations in Burgas Bay (+ 1 in Sozopol Bay); however, there is always at least one value/station, so it would be possible to impute the missings - in this case, through **mean value imputation**. While it is possible to use a more sophisticated method (I initially read up on MCMC and tried it with package mice & friends), I don't see much difference, so I'd rather stick with the simpler solution.  
