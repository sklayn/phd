...
Try another thing - manually, unfortunately: this same nightmare, but colored by the values of each model term. Since there are 4 terms, I'll make a 2 x 2 plot grid. There will be one (manual) facet per term, with the y-axis present only on the left-most plots. 

```{r composite_plot_relative_taxon_contrib_top_glm_env_sand}
p1 <- ggplot(abnd.top.sp.glms.env.red.sand,
             mapping = aes(x = log_y_min(count), y = species, colour = PO4)) + 
  geom_point() +
  theme(legend.position = "top") + 
  labs(x = "", y = "")

p2 <- ggplot(abnd.top.sp.glms.env.red.sand,
             mapping = aes(x = log_y_min(count), y = species, colour = seston)) + 
  geom_point() +
  theme(legend.position = "top", 
...
p3 <- ggplot(abnd.top.sp.glms.env.red.sand,
             mapping = aes(x = log_y_min(count), y = species, colour = LUSI)) + 
  geom_point() +
  theme(legend.position = "top", 
        axis.text.y=element_blank(),
        axis.title.y=element_blank(),
...
p4 <- ggplot(abnd.top.sp.glms.env.red.sand,
             mapping = aes(x = log_y_min(count), y = species, colour = gravel)) + 
  geom_point() +
  theme(legend.position = "top", 
        axis.text.y=element_blank(),
        axis.title.y=element_blank(),
...
pp <- gridExtra::arrangeGrob(egg::ggarrange(plots = list(p1, p2, p3, p4), ncol = 4),
                grid::textGrob("Abundance"), heights=c(30,1))
grid::grid.newpage()
grid::grid.draw(pp)

# ggsave(here(figures.dir, "monstr.png"), width = 25, height = 20, units = "cm", dpi = 300)
```
