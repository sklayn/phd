...
sp_all %>% 
  ...
  filter(`(Intercept).y` < 0.05) %>%
  arrange(`(Intercept).y`, desc(`(Intercept).x`))

sp_all %>% 
  select(species, contains("4")) %>%
  filter(lvm.clusters.sand4.y < 0.05) %>%
  arrange(lvm.clusters.sand4.y, desc(lvm.clusters.sand4.x))



sort(glms.lvm.sand.aov$uni.test[2,]/glms.lvm.sand.aov$table[2, 3], decreasing = T)
