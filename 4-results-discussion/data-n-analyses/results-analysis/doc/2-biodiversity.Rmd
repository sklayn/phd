...
## save & clean up
cowplot::save_plot(file.path(figures.dir, "dplus_lambda_combined_sand.png"), 
...
```
  ...
  The summary curves will be calculated on **data averaged by year** - this way, there will hopefully be enough replication to reduce the over-dominance of large mollusks etc. in the biomass (they are rarely dominant in the abundance - more often represented by 1-2 individuals/sample). Also, will calculate the partial dominance curves as well, for comparison.  
...
plot.summary.abc.sand <- ggplot(abc.summary.sand[[1]], aes(x = sp_rank)) + 
  geom_line(aes(y = cum_abnd), colour = "skyblue") + 
  geom_line(aes(y = cum_biomass), colour = "orange") +
  scale_x_log10() +
...
(plot.summary.abc.sand <- plot.summary.abc.sand + text_size(text.x = rel(1.2), text.y = rel(1.2), legend.text = rel(1.1), title.x = rel(1.1), title.y = rel(1.1), strip.x = rel(1.1), strip.y = rel(1.1))
)
...
plot.pd.sand <- ggplot(pd.summary.sand, aes(x = sp_rank)) + 
  geom_line(aes(y = partial_abnd), colour = "skyblue") + 
  geom_line(aes(y = partial_biomass), colour = "orange") +
  scale_x_log10() +
  facet_grid(year ~ station, labeller = labeller(station = as_labeller(stations.sand))) + 
  labs(x = "Species rank (log)", y = "%")

## modify text size a little
(plot.pd.sand <- plot.pd.sand + text_size(text.x = rel(1.2), text.y = rel(1.2), legend.text = rel(1.1), title.x = rel(1.1), title.y = rel(1.1), strip.x = rel(1.1), strip.y = rel(1.1))
)
