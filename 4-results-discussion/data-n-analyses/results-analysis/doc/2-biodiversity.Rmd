...
source(here(functions.dir, "alpha_diversity.R"))
source(here(functions.dir, "graphical_analyses.R"))
```
...
## define a custom transformation and its inverse for the y axis of the ABC curves (modified logistic transformation) using the scales package (for ggplot graphics).
...
tax.dist.all <- taxa2dist(master.tax.list, varstep = TRUE)
...
(tax.distinctness.sand <- taxondive(select(zoo.abnd.sand, -c(station:replicate)), tax.dist.all)
)
...
tax.dist.sand.current <- as.matrix(tax.dist.all)[which(rownames(as.matrix(tax.dist.all)) %in% colnames(zoo.abnd.sand)), which(colnames(as.matrix(tax.dist.all)) %in% colnames(zoo.abnd.sand))]
...
```{r plot_w_sand}
plot.w.sand <- ggplot(abc.sand[[2]], aes(x = station, y = w, colour = factor(year))) + 
  geom_boxplot() + 
  scale_color_viridis(option = "viridis", begin = 0.5, discrete = TRUE, name = "") +
...
(plot.w.sand <- plot.w.sand + text_size(text.x = rel(1.2), text.y = rel(1.2), legend.text = rel(1), title.x = rel(1.1), title.y = rel(1.1))
)

## and save this.. 
ggsave(here::here(figures.dir, "w_sand.png"), 
       plot.w.sand, 
       width = 15, units = "cm", dpi = 300)
  
```

  W > 0 for all samples => the biomass curve is above the abundance curve, so none of the stations/replicates can be considered terribly disturbed. Still, W is pretty close to 0 (< 0.5 for all stations/years). The closer to 0 the statistic gets, the closer the 2 curves are; here, S5-Agalina comes closest, but still not too close. Nowhere is there complete dominance of one species in the biomass, perfectly even distribution of the abundance among species (W = 1, or tending towards it), nor the opposite (W = -1). In short, the structure of the communities is more or less even and normal, with variations characteristic of macrozoobenthic samples/communities. 
...
```
  ...
  Finally, make a combined plot of W and ABC to use in the text; there are already a million plots so I need to conserve space as much as possible.. 
```{r plot_composite_abc_w_sand}
(plot.abc.w.combined.sand <- cowplot::plot_grid(plot.summary.abc.sand,
                                                plot.w.sand + theme(legend.position = "top"), 
                                                labels = c("A", "B"), ncol = 2, rel_widths = c(2,1))
)

## save & clean up
cowplot::save_plot(here::here(figures.dir, "abc_w_combined_sand.png"), 
                   plot.abc.w.combined.sand,
...
(tax.distinctness.zostera <- taxondive(select(zoo.abnd.zostera, -c(station:replicate)), tax.dist.all)
)
...
## subset the large dist object (taxonomic distance b/n species) from before, to only the currently present species - in a very clumsy, very ugly way
tax.dist.zostera.current <- as.matrix(tax.dist.all)[which(rownames(as.matrix(tax.dist.all)) %in% colnames(zoo.abnd.zostera)), which(colnames(as.matrix(tax.dist.all)) %in% colnames(zoo.abnd.zostera))]
...
```
  ...
  We'll plot W, because there are too many stations & replicates and the curves will be unreadable.   
```{r plot_w_zostera}
plot.w.zostera <- ggplot(abc.zostera[[2]], aes(x = station, y = w, colour = factor(year))) + 
  geom_boxplot() + 
  scale_color_viridis(option = "viridis", begin = 0.5, discrete = TRUE, name = "") +
  scale_x_discrete(name = "Station", 
...
```
  ...
  Z4 in 2014 - worse condition - similar to Z3 -> could be because of heavy rains on 2014-07-15/16 (samples from Z4 taken later than those at the other stations, on 2014-07-18 - after the rains). Not sure if relevant, though - how fast can the communities react? Was it enough of a disturbance?   
  ...
  Make a combined plot of W and ABC in the seagrasses. 
```{r plot_composite_abc_w_zostera}
(plot.abc.w.combined.zostera <- cowplot::plot_grid(plot.summary.abc.zostera,
                                                plot.w.zostera + theme(legend.position = "top"), 
                                                labels = c("A", "B"), ncol = 2, rel_widths = c(2,1))
)

## save & clean up
cowplot::save_plot(here::here(figures.dir, "abc_w_combined_zostera.png"), 
                   plot.abc.w.combined.zostera,
                   base_width = 10, base_height = 6, dpi = 300)
...
## dimensions should be 48 x 206 (6 metadata + 200 species columns); all species columns should be numeric.
...
zoo.abnd.2012 <- zoo.abnd.2012 %>% 
  mutate(station = factor(station, levels = c("Konski1", "Konski2", "Ribka1", "Ribka2", "Gradina1", "Gradina2")), 
         site = factor(site, levels = c("Konski", "Ribka", "Gradina"))) %>%
...
## result should have dimensions 48 x 64 (58 species + 6 metadata columns) 
zoo.abnd.2012 <- zoo.abnd.2012 %>% 
...
## add the metadata columns (row order is not changed, so possible) 
(alpha.div.2012 <- bind_cols(zoo.abnd.2012 %>% select(site:replicate), alpha.div.2012)
)
```
  Summarize these diversity indices by site & habitat.
```{r summarize_alpha_diversity_2012}
## STILL haven't learned how to use the functions from package sjPlot for printing pretty (and easy!) tables...   
alpha.div.2012 %>% 
  group_by(site, habitat) %>%
  summarise(h_mean = mean(shannon_h), h_sd = sd(shannon_h), 
            j_mean = mean(evenness_j, na.rm = TRUE), j_sd = sd(evenness_j, na.rm = TRUE),
            h_eff_mean = mean(h_effective), h_eff_sd = sd(h_effective), 
            nb_replicates = n()
            ) %>% 
...
sac.2012 <- specaccum(select(zoo.abnd.2012, -c(site:replicate)))

plot(sac.2012, 
     xlab = "Samples", ylab = "Cumulative species richness", 
     ci.type = "polygon", ci.col = "light blue")
```
  Again, about 10 (maybe a little more) samples seem to be enough to capture most of the biodiversity in the study area (in terms of species richness) - this is where the curve starts to smooth out (slope < 1). In reality, there are 4 samples/station & habitat, or 8 samples/site & habitat - clearly not enough. Also supported by the horrifying species richness values (sometimes only 1 species??). I'm putting it down as a result of my learning the taxonomy, methods etc.  
...
## this function is based on incidences in sample sites - presence/absence data; gives a single estimate for a collection of sample sites
specpool(select(zoo.abnd.2012, -c(site:replicate)), 
         pool = zoo.abnd.2012$habitat, 
         smallsample = TRUE)

## this function is quantitative - based on abundances on single sample site
estimateR(select(zoo.abnd.2012, -c(site:replicate)))
```
  ...
  In reality, the two habitats were not really distinct - the sand samples were taken too close to the seagrass meadows, in the case of Gradina - in unvegetated patches INSIDE the meadows, so I'm not sure if a distinction is possible/meaningful from this data.     
  
  Then - at the different sites.   
```{r sp_pool_site_2012}
## this function is based on incidences in sample sites - presence/absence data; gives a single estimate for a collection of sample sites
specpool(select(zoo.abnd.2012, -c(site:replicate)), 
         pool = zoo.abnd.2012$site, 
         smallsample = TRUE)
...
```
  When all sites/habitats are pooled, 83 % of the species pool are estimated to be captured by this dataset based on 2nd order jacknife (the most severe measure), and 90 % - based on Chao & bootstrap - the most tolerant measures. While this *looks* fine, I don't quite believe it - especially given the later samplings at Gradina, where the species richness is never < 25 in any one sample..    
...
## calculate the taxonomic distinctness indices for the abundance dataset
(tax.distinctness.2012 <- taxondive(select(zoo.abnd.2012, -c(site:replicate)), tax.dist.all)
)
...
s.l.dplus.2012 <- bind_cols(S = tax.distinctness.2012$Species,
                            ...
                            site = zoo.abnd.2012$site, 
                            habitat = zoo.abnd.2012$habitat)

plot.dplus.2012 <- ggplot(s.l.dplus.2012, aes(x = S, y = Dplus, color = site)) +
  scale_color_brewer(palette = "Set2", 
                     labels = c("K", "R", "G")) +
  geom_point(alpha = 0.85, size = 2) +
  xlim(0, 100) + 
...
```
  Again, there are too few species in the samples for this to be a meaningful analysis. Many of the samples would have a disturbed community structure if the results could be trusted - esp. G in seagrass, and also somewhat in sand. Overall, K samples which have the highest species richness in both sand and in seagrass are the closest to the average expected Delta+.  
  
  Now plot the observed Lambda values, with the theoretical (simulated) confidence intervals.  
```{r plot_lambda_2012}
plot.lambda.2012 <- ggplot(s.l.dplus.2012, aes(x = S, y = Lambda, color = site)) +
  scale_color_brewer(palette = "Set2",
                      labels = c("K", "R", "G")) +
  geom_point(alpha = 0.85, size = 2) +
  xlim(0, 100) + 
...
```
  Same story here: very high variability between samples, mostly very low values of Lambda - because the few species that are present are of similar taxonomic groups. Site K is again the closest to the expected average values.  
...
(abc.2012 <- abc(zoo.abnd.2012.long, zoo.biomass.2012.long, 
                 abnd.val = sp_count, biomass.val = sp_biomass, 
                 site, station, habitat, replicate)
)
## result is a list: 1) grouped df containing the curves (long), 2) summarized df with the W statistic per replicate - the distance between the 2 curves  

```
  
  We'll plot W, because there are too many sites & replicates and the curves will be unreadable.   
```{r plot_w_2012}
plot.w.2012 <- ggplot(abc.2012[[2]], aes(x = site, y = w, colour = factor(habitat))) + 
  geom_boxplot() + 
  scale_color_viridis(option = "viridis", begin = 0.25, discrete = TRUE, name = "", labels = c("sand", "Zostera")) +
...
## modify text size a little 
(plot.w.2012 <- plot.w.2012 + text_size(text.x = rel(1.2), text.y = rel(1.2), legend.text = rel(1), title.x = rel(1.1), title.y = rel(1.1))
)

## and save this.. 
ggsave(here::here(figures.dir, "w_2012.png"), 
       plot.w.2012, 
       width = 15, units = "cm", dpi = 300)
  
```

  W > 0 for most samples, but the variability is incredibly high, esp. at site G (+ an incredible difference between sand and seagrass there) - hardly surprising; those samples contain the lowest number of species. Just as biased as the previous analysis.  
  
  Plot the summary ABC curves - per site and habitat. 2 rows (for the 2 years) + 3 columns (sites) in the plot.  
```{r summarize_abnd_biomass_2012}
## summarize the abundance and biomass data per year (average)  
(summary.abnd.2012 <- zoo.abnd.2012.long %>%
   group_by(site, habitat, species) %>%
   summarize(mean_count = mean(sp_count))
)
...
abc.summary.2012.long <- abc.summary.2012[[1]] %>% 
  gather(key = variable, value = value, -c(site:sp_rank)) %>%
  mutate(variable = case_when(variable == "cum_abnd" ~ "abundance", 
                              variable == "cum_biomass" ~ "biomass"))

## make named character vectors for the custom station & habitat labels   
sites.2012 <- c("K", "R", "G")
names(sites.2012) <- as.character(unique(abc.summary.2012[[1]]$site))
...
plot.summary.abc.2012 <- ggplot(abc.summary.2012.long, 
  ...
  facet_grid(habitat ~ site, 
             labeller = labeller(site = as_labeller(sites.2012),
...
```
  Spectacularly bad.. For what it's worth - all the seagrass samples have curves situated close together and sometimes crossing - indicative of slight disturbance, or would be if the underlying data were any good. The sand samples show more or less the same pattern - slight disturbance; site G has the widest distance between curves - but also, let's not forget, the largest bias because some samples contain only 1-2 species..

  Make a combined plot of W and ABC in the Sozopol Bay habitats. 
```{r plot_composite_abc_w_2012}
(plot.abc.w.combined.2012 <- cowplot::plot_grid(plot.summary.abc.2012,
                                                plot.w.2012 + theme(legend.position = "top"), 
                                                labels = c("A", "B"), ncol = 2, rel_widths = c(2,1))
)

## save & clean up
cowplot::save_plot(here::here(figures.dir, "abc_w_combined_2012.png"), 
                   plot.abc.w.combined.2012,
                   base_width = 10, base_height = 6, dpi = 300)
...
plot.pd.2012 <- ggplot(pd.summary.2012.long, aes(x = sp_rank, y = value, colour = variable)) + 
  ...
  # facets & labels
  facet_grid(habitat ~ site, 
             labeller = labeller(station = as_labeller(sites.2012), 
                                 habitat = as_labeller(habitats.2012))) + 
  labs(x = "Species rank (log)", y = "%")
