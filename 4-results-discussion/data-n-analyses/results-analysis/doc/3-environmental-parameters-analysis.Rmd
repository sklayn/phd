...
To reiterate - the automatic report **fails** at the attempt to describe the dimensions - that's because there is nothing to describe; the original data only has 1 value per station - not really appropriate for PCA, which attempts to describe variability after all (what variability with only 1 value?).  
...
Visualize the correlations between the water parameters and LUSI.. 
```{r water_sand_all_LUSI_cors_visualization}
ggcorr(water.sand.all.LUSI %>% select(-c(station, year, month)), 
       label = TRUE, label_round = 2, label_size = 3)

```
Not very significant, except between N species etc. which is hardly surprising.. Probably because for all the chemical concentrations, there is only one LUSI.  
