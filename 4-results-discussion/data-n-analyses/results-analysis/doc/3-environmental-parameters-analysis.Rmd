...
## ... and import the pseudo-manplated-long-term data for the seagrasses
...
## convert stations into factor, because for some stations there are more observations than for others, which causes the Investigate function to freak out later
water.zostera.LT.red.scaled <- water.zostera.LT.red.scaled %>% 
  mutate(station = factor(station, levels = c("Poda", "Otmanli", "Vromos", "Gradina", "Ropotamo")))
...
plot.water.sand.all.red.pca <- fviz_pca_biplot(water.sand.all.red.pca,
                                               ...
                                               repel = TRUE,
                                               alpha.ind = 0.75,  
                                               title = "Water - sand stations (2013-14)")

## fix the color scale
plot.water.sand.all.red.pca + scale_color_viridis(discrete = TRUE, direction = -1)
```

```{r plot_sediments_sand_all_red_pca}
...
plot.water.zostera.red.pca <- fviz_pca_biplot(water.zostera.red.pca,
                                              ...
                                              repel = TRUE,
                                              alpha.ind = 0.75,
                                              title = "Water - seagrass stations (2013-14)")

## fix the color scale
plot.water.zostera.red.pca + scale_color_viridis(discrete = TRUE, direction = -1)
```

```{r plot_sediments_zostera_all_red_pca}
