...
(water.sand <- water.params.summary %>% 
   filter(station %in% c("Kraimorie", "Chukalya", "Akin", "Sozopol", "Agalina", "Paraskeva")) %>% 
   ungroup() %>%
   mutate(station = factor(station, levels = c("Kraimorie", "Chukalya", "Akin", "Sozopol", "Agalina", "Paraskeva")))
)
...
## filter (manually) to get only the zostera 2013-2014 stations
(water.zostera <- water %>% 
...
Import the sediment data (it also contains the heavy metals).   
```{r import_sediment_data_zostera}
...
Starting with water column crap.  
```{r en_water_zostera}
## get only the zostera stations from the water column dataset
(water.zostera <- water.params.summary %>% 
   filter(station %in% c("Poda", "Otmanli", "Vromos", "Gradina", "Ropotamo")) %>% 
   ungroup() %>%
   mutate(station = factor(station, levels =  c("Poda", "Otmanli", "Vromos", "Gradina", "Ropotamo")))
)
...
en.water.st.zostera <- glmnet(x = as.matrix(water.zostera %>% select(chl_a:seston)), 
                           y = water.zostera %>% pull(station), 
                           family = "multinomial", type.multinomial = "grouped")

plot(en.water.st.zostera, xvar = "lambda", label = TRUE, type.coef = "2norm")


cvfit.water.st.zostera <- cv.glmnet(x = as.matrix(water.zostera %>% select(chl_a:seston)),
                                 y = water.zostera %>% pull(station), 
                                 family="multinomial", type.multinomial = "grouped")
...
```
Goody - NO significant variables according to this analysis...   
Let's try all of them in the model, then.  
```{r test_glm_en_water_zostera}
summary(glm(as.numeric(station) ~ chl_a + Ninorg + NH4 + NO3 + Ntotal + PO4 + seston, data = water.zostera))
```
So seston again, and at the boundary of significance...   

OK - let's try again with the **sediment parameters minus the heavy metals**.   
However, for the seagrass stations, this doesn't work, because for Vromos and Ropotamo there is a single observation each - not allowed by the algorithm...  
```{r en_sediments_zostera}
## exclude Vromos and Ropotamo, just for the fun of it 
sediments.zostera.sub <- sediments.zostera %>% 
  filter(station != "Vromos", station != "Ropotamo") %>% 
  # make station a factor
  mutate(station = factor(station, levels = c("Poda", "Otmanli", "Gradina")))
  
en.sediments.st.zostera <- glmnet(x = as.matrix(sediments.zostera.sub %>% select(TOM:silt_clay)), 
                           y = sediments.zostera.sub %>% pull(station), 
                           family = "multinomial", type.multinomial = "grouped")

plot(en.sediments.st.zostera, xvar = "lambda", label = TRUE, type.coef = "2norm")

cvfit.sediments.st.zostera <- cv.glmnet(x = as.matrix(sediments.zostera.sub %>% select(TOM:silt_clay)),
                                 y = sediments.zostera.sub %>% pull(station), 
                                 family="multinomial", type.multinomial = "grouped")
...
In this case, I am going to go back and look at the sediment data for the rest of the stations (there were 2 stations in each zostera meadow at each sampling, and there is data for those, too).  
