...
perm.eq.sand <- paste("dist.matrix.sand", paste(names(env.data.ordi.sand), collapse = "+"), sep = " ~ ")

## set blocks for the permutations
perm.sand <- how(nperm = 999)
setBlocks(perm.sand) <- zoo.abnd.sand$station
  
adonis(as.formula(perm.eq.sand), data = env.data.ordi.sand, permutations = perm.sand)

adonis2(as.formula(perm.eq.sand), data = env.data.ordi.sand, strata = kmeans.clusters.sand)
```
...
The station grouping **is** significant after all, according to ANOSIM.  
Save it!  
...
The year grouping is not significant, according to ANOSIM.  
