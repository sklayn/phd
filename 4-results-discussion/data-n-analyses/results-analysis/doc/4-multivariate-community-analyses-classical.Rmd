...
Save this - although it's wrong to do this, I don't want to repeat this disgrace of a table construction again. I'll just import it and make a note on how I arrived at it in the next notebook.  
```{r save_env_data_sand}
write.csv(env.data.sand, 
          here(save.dir, "env_data_ordinations_sand.csv"))
```
...
* group 2 (S5): “Infralittoral mixed sediments with diverse fauna” (ILitMSVar) - also one of the national subtypes, from the latest report, but without ecological state limits - with many species having significant and really close contributions to the community composition. The dominants are *Microdeutopus versiculatus*, *Protodorvillea kefersteini*, *Melita palmata*, *Eurydice dollfusi*, etc. It really is pretty unique in composition - the closest would be S3, but the sediment composition distingishes it enough..  
...
(biotope.labs.sand <- as_tibble(scores(mds.sand, display = "sites")) %>%
...
Do a within-group SIMPER on the **biomasses**, too.. (get the biomass dominants in each group).  
```{r simper_biomass_wn_group_mds_clusters_st_sand}
## import biomasses in seagrass 
(zoo.biomass.sand <- read_csv(here(save.dir, "biomass_sand_orig_clean.csv"))
)

## perform the SIMPER
(simper.biomass.wn.gr.sand <- simper_within_group(zoo.biomass.sand %>% select(-c(station:replicate)), 
                                                  group = kmeans.clusters.sand)
)
```

So the groups from before are still distinct, and still the same - but the community dominants are shuffled a little.. Normal - as I've said before, biomasses are really skewed in favour of the mollusks with their shells.. 

Save for later! 
```{r save_simper_biomass_wn_gr_mds_clusters_sand}
write_rds(simper.biomass.wn.gr.sand, 
          here(save.dir, "simper_biomass_wn_gr_mds_clusters_sand.RDS"))
```
...
After some reflection, this is all good and nice, but completely meaningless - most of the variability is gone from the environmental dataset (water column averages), or there isn't enough to begin with (sediment data). It's bound to be even worse with the seagrass data. So in the end, I'm going to drop it altogether.    

Save the PERMANOVA results, even though I'm not going to use it.  
```{r save_permanova_env_params_sand}
write_rds(permanova.env.sand, 
          here(save.dir, "permanova_env_params_sand.RDS"))
```

##### **bioenv - environmental parameters, sand stations**  
This is the last thing I'm going to try - bioenv (I think it's equivalent to the PRIMER function of the same name).  
It selects all possible subsets of environmental variables, finds the correlation between community dissimilarities and environmental distances, and for each size of subsets, saves the best result. The idea is to get the most parsimonious model that best explains the observed community differences.   
I don't expect this to actually perform better (same underlying environmental data, same problems with grossly oversimplified variables).   
```{r bioenv_sand}
## use the same equation and data as for the PERMANOVA above
bioenv.sand <- bioenv(as.formula(permanova.eq.sand), 
                      data = env.data.ordi.sand, 
                      method = "spearman", 
                      metric = "euclidean")
...
Well, as expected: one more confirmation that the observed community differences are best explained by eutrophication parameters (NH4, or other water column parameters depending on the distance metric used) AND by sediment properties (grain size, organic matter,..).  

Save this in case I decide I need it.. 
```{r save_bioenv_sand}
write_rds(bioenv.sand, 
          here(save.dir, "bioenv_sand.RDS"))
```
...
Import zoobenthic abundance data (cleaned and prepared, from the output directory).  
...
Save this again - to avoid repeating this disgrace of a table construction again. I'll just import it and make a note on how I arrived at it in the next notebook.  
```{r save_env_data_zostera}
write.csv(env.data.zostera.all, 
          here(save.dir, "env_data_ordinations_zostera.csv"))
```
...
Do a within-group SIMPER on the **biomasses**, too.. (get the biomass dominants in each group).  
```{r simper_biomass_wn_group_mds_clusters_st_zostera}
## import biomasses in seagrass 
(zoo.biomass.zostera <- read_csv(here(save.dir, "biomass_zostera_orig_clean.csv"))
)

## perform the SIMPER
(simper.biomass.wn.gr.zostera <- simper_within_group(zoo.biomass.zostera %>% select(-c(station:replicate)), 
                                                     group = clusters.st.zostera)
)
```

Great, so suspicions more or less confirmed: there aren't very great community distinctions between stations.. The biomass dominants are mollusks, most commonly *Bittium reticulatum*, *Rissoa membranacea*, *Loripes orbiculatus*.. Exception: st. Z3-Vromos: dominated by *A. diadema*, *R. membranacea*, *S. filicornis*..   
There is a shift towards species more tolerant to organic pollution/accumulation in the inner Burgas Bay stations - *Polydora ciliata*, *Prionospio cirrifera*, oligochaetes.  
However, as a whole, I think it would be appropriate to consider the seagrass-associated zoobenthic species as one single community.    

Save this biomass SIMPER, too.  
```{r save_simper_biomass_wn_gr_mds_clusters_st_zostera}
write_rds(simper.biomass.wn.gr.zostera, 
          here(save.dir, "simper_biomass_wn_gr_mds_clusters_st_zostera.RDS"))
```


##### **PERMANOVA - environmental parameters, seagrass stations**  
Same as for the sand stations.. 
Make a distance matrix of the communities, and check multivariate homogeneity of groups dispersions (variances) - a PERMANOVA assumption.  
```{r check_permanova_assumptions_zostera}
## make distance matrix of the seagrass communities (square root transformed)
dist.matrix.zostera <- vegdist(zoo.abnd.zostera %>% 
                                 select(-c(station:replicate)) %>%
                                 mutate_all(sqrt))

## check the dispersion between groups - using the clusters from the MDS
(betadisper.zostera <- betadisper(dist.matrix.zostera, 
                                  group = clusters.st.zostera)
)

anova(betadisper.zostera)

```

So again there are significant differences in dispersion between groups that might also contribute to any significant effect detected by PERMANOVA.. 

```{r permanova_env_params_zostera}
## construct equation for PERMANOVA. NB remove the secchi depth, because it has NaNs (for Vromos), which prevent the analysis from running - unequal matrices.. 
permanova.eq.zostera <- paste("dist.matrix.zostera", 
                              paste(names(env.data.ordi.zostera %>% select(-secchi)), collapse = "+"), 
                              sep = " ~ ")

## set blocks for the permutations
permutations.zostera <- how(nperm = 999)
setBlocks(permutations.zostera) <- clusters.st.zostera
 
## run the PERMANOVA with the parameters defined above  
(permanova.env.zostera <- adonis(as.formula(permanova.eq.zostera),
                                 data = env.data.ordi.zostera, 
                                 permutations = permutations.zostera)
)
```

All right, so the most significant parameters are again eutrophication/pollution parameters - LUSI, and sediment grain size (sand and silt), and also seagrass density (shoot count).  

Save the PERMANOVA for the seagrass stations.  
```{r save_permanova_env_params_zostera}
write_rds(permanova.env.zostera, 
          here(save.dir, "permanova_env_params_zostera.RDS"))
```

##### **bioenv - environmental parameters, seagrass stations**  
So, for consistency, here is bioenv on the environmental parameters at the seagrass stations.  
It selects all possible subsets of environmental variables, finds the correlation between community dissimilarities and environmental distances, and for each size of subsets, saves the best result. The idea is to get the most parsimonious model that best explains the observed community differences.   
I don't expect this to actually perform better (same underlying environmental data, same problems with grossly oversimplified variables).   
```{r bioenv_zostera}
## use the same equation and data as for the PERMANOVA above
bioenv.zostera <- bioenv(as.formula(permanova.eq.zostera), 
                         data = env.data.ordi.zostera, 
                         method = "spearman", 
                         metric = "euclidean")

bioenv.zostera
summary(bioenv.zostera)

```

Interestingly, here the bioenv procedure suggests that the sediment grain size (% sand) and the seagrass shoot density are the parameters that best explain the observed community differences. Could be - I wouldn't say there are much differences at all, see the MDS..   

Save this, too.  
```{r save_bioenv_zostera}
write_rds(bioenv.zostera, 
          here(save.dir, "bioenv_zostera.RDS"))
```
...
#### **Sozopol Bay, 2012**  
Import zoobenthic abundance data (cleaned and prepared, from the output directory).  
```{r import_zoo_abnd_2012}
zoo.abnd.2012 <- read_csv(here(save.dir, "abnd_2012_orig_clean.csv"))

## convert site to factor (better safe than sorry later, when the sites are not plotted in the order I want them)
(zoo.abnd.2012 <- zoo.abnd.2012 %>% 
    mutate(site = factor(site, levels = c("Konski", "Ribka", "Gradina")))
)
```

Also import the biomass (again, from the save directory).  
```{r import_zoo_biomass_2012}
zoo.biomass.2012 <- read_csv(here(save.dir, "biomass_2012_orig_clean.csv"))
...
##### **Ordination (nMDS) on Sozopol Bay stations.**  
These are all functions from library vegan.  
```{r mds_zoo_2012}
## the vegan MDS function transforms the data (fourth root) and applies Bray-Curtis dissimilarity by default.   
mds.2012 <- metaMDS(zoo.abnd.2012 %>% select(-c(site:replicate)), 
                    distance = "bray", 
                    autotransform = TRUE, trymax = 50)

mds.2012
```

Well this is shit, as expected... To obtain a convergent solution, I had to increase the maximum number of tries (random starts of the algorithm) to 50... And the stress is still high, 0.21.
...
## goodness-of-fit plot 
## first, plot the nMDS ordination with sites
plot(mds.2012, display = 'sites', type = 't', main = 'Goodness of fit') 

## then, add the points with size reflecting goodness of fit (smaller = better fit)
points(mds.2012, display = 'sites', cex = goodness(mds.2012)*200) 

```


Save the MDS for the Sozopol Bay sites.  
```{r save_mds_2012}
write_rds(mds.2012, 
          here(save.dir, "mds_2012.RDS"))
```


Plot the MDS, colouring the points by site.. 
```{r plot_mds_sites_2012}
## plot the MDS, using my custom ggplot function
plot.mds.2012.sites <- plot_mds(mds.2012, groups = zoo.abnd.2012 %>% pull(site))

## change the colour scheme & labels to keep overall consistency.. 
plot.mds.2012.sites <- plot.mds.2012.sites +
      scale_color_brewer(palette = "Set2", name = "site", 
                         labels = c("K", "R", "G"))

## save the plot
ggsave(here(figures.dir, "mds_sites_2012.png"), 
       plot.mds.2012.sites, 
       width = 15, units = "cm", dpi = 300)

## add 95% confidence intervals, for the fun of it 
plot.mds.2012.sites + stat_ellipse()
```
Adding in the 95% confidence interval to make the mess somewhat clearer, it kinda-sorta seems that site K MIGHT be considered a little different - but if you squint really really hard. Mostly, I don't think the data allows to distinguish the sites; the differences in community structure are likely to be due to the habitats (with the sites contributing some secondary effects) - see the MDS plot coloured by habitat below.

```{r plot_mds_habitats_2012}
## plot the MDS, using my custom ggplot function
plot.mds.2012.habitats <- plot_mds(mds.2012, groups = zoo.abnd.2012 %>% pull(habitat))

## change colour to according to habitat (sand/zostera), and change labels
(plot.mds.2012.habitats <- plot.mds.2012.habitats +
      scale_color_manual(values = c("orange", "light green"), name = "habitat", 
                         labels = c("sand", "Zostera"))
)

## save the plot
ggsave(here(figures.dir, "mds_habitats_2012.png"), 
       plot.mds.2012.habitats, 
       width = 15, units = "cm", dpi = 300)
```

Now *this* picture is interesting - the seagrass samples are found compactly in the middle, with the sand samples surrounding them.. If we check the goodness-of-fit plot, the seagrass samples are also better represented (-> smaller circles). The sand samples contain very few species and individuals, therefore they are more dispersed - hence the high differences.. 
I want to stress once again that the most likely reason for the poor species diversity, etc. is my own inexperience - I was still learning then, and it's highly likely that I missed a lot.. Should have maybe kept the samples, then I could have come back to them - but again, due to inexperience I didn't know..  

All in all, it appears that in the preliminary experiment, the sand communities in Sozopol Bay are somewhat distinct from the seagrass ones, but not clearly so. One possible reason could be that the sand samples were not taken far enough from the seagrass to be "free" from its influence. Especially true in the case of site G (Gradina) - the unvegetated samples were taken from bare patches within the seagrass meadow itself (because the meadow is very large).  


Make a combined MDS plot in ggplot - NOT using my nice custom function, because it doesn't allow for easy mapping of two factors - to colours and shapes, for example.
```{r plot_mds_combined_2012}
## extract MDS scores for 2012
mds.scores.2012 <- as_data_frame(scores(mds.2012, display = "sites"))

## add the sites and habitats (everything is in the same order)
(mds.scores.2012 <- mds.scores.2012 %>%
    mutate(site = zoo.abnd.2012 %>% pull(site), 
           habitat = zoo.abnd.2012 %>% pull(habitat))
)

## plot 
plot.mds.2012 <- ggplot(mds.scores.2012, 
                        aes(x = NMDS1, y = NMDS2, colour = site, shape = habitat)) + 
  geom_point(size = 2, position = "identity")

## fix colour scheme, shapes and labels
(plot.mds.2012 <- plot.mds.2012 + 
    scale_shape_manual(values = c(19, 8), labels = c("sand", "Zostera")) + 
    scale_color_brewer(palette = "Set2", labels = c("K", "R", "G"))
)

## save the plot 
ggsave(here(figures.dir, "mds_2012.png"), 
       plot.mds.2012, 
       width = 15, units = "cm", dpi = 300)
```

When we examine (visually) the differences between habitats - not sure if this is a valid approach - it's clear that any strong site differences are due to the sand samples. The seagrass samples form a much more compact group at the center. Again, I think this is caused by the particular shortcomings of the 2012 samples.    
```{r plot_mds_2012_facets}
plot.mds.2012 + facet_wrap(~habitat)

```

Determine the optimal number of clusters by **k-means clustering** on the abundance data.
```{r kmeans_2012} 
## with Hellinger transformation of the data (because k-means uses Euclidean distances which don't work well for ecological data).
kmeans.casc.2012 <- cascadeKM(decostand(zoo.abnd.2012 %>% select(-c(site:replicate)),
                                        method = "hellinger"),
                              inf.gr = 2, sup.gr = 6) ## I want at least 2 groups and at most 6 (the number of original stations)

plot(kmeans.casc.2012, sortg = TRUE)
```

This result suggests 3 groups, but they cut over each other, again.. It's a terrible mess.     
Let's go with 3 and check them. 
```{r kmeans_clusters_2012}
## get the partition in 3 groups 
(kmeans.clusters.2012 <- as.vector(kmeans.casc.2012$partition[, 2]))

## plot the MDS, colouring by these suggested groups.. 
plot_mds(mds.2012, groups = as.factor(kmeans.clusters.2012))

```

As expected, it's no good at all.. Well, I had to check. 
I'm sticking with sites and habitats as groups, then.

##### **ANOSIM - Sozopol Bay, 2012**
Let's check the significance of the groupings.. 
```{r anosim_sites_2012}
anosim.sites.2012 <- anosim(zoo.abnd.2012 %>% 
                              select(-c(site:replicate)) %>% 
                              mutate_all(sqrt),
                            grouping = zoo.abnd.2012 %>% pull(site))

summary(anosim.sites.2012)
```
Kind of average, and on the weak side, if you ask me.. R = 0.39, supposed to be significant. 

Now between habitats.. 
```{r anosim_habitats_2012}
anosim.habitats.2012 <- anosim(zoo.abnd.2012 %>% 
                                 select(-c(site:replicate)) %>% 
                                 mutate_all(sqrt),
                               grouping = zoo.abnd.2012 %>% pull(habitat))

summary(anosim.habitats.2012)
```
And this is even weaker - R = 0.27 - probably because of the really, really high variability of the sand samples, which, again, is an artefact of processing and hence not to be trusted at all. 

##### **SIMPER - Sozopol Bay, 2012**
I'm going to do a quick couple of SIMPERs on this, despite the above ANOSIM results... 
```{r simper_bn_gr_2012_habitats}
simper.bn.gr.2012.habitats <- simper(zoo.abnd.2012 %>%
                                       select(-c(site:replicate)) %>%
                                       mutate_all(sqrt),
                                     group = zoo.abnd.2012 %>% pull(habitat))

simper.bn.gr.2012.habitats
summary(simper.bn.gr.2012.habitats)
```
The overall difference is 73%. The species/taxa with the highest contributions to the differences between habitats (account for 70% of the overall difference) are B.reticulatum (Z > S); Oligochaeta (Z > S, but not by much); L.orbiculatus (Z > S); M.lineatus (Z > S); R.membranacea (Z >> S); C.gallina (S >> Z); C.capitata (Z > S); A.improvisus (Z > S); H.filiformis (Z > S, but very close); Rissoa sp. (Z >> S). 
This is hardly very surprising at all - seagrass samples have higher abundances - in sand samples, sometimes there were only several individual organisms.. Practically the only species that's more abundant in sand is Chamelea gallina. The others are all either herbivores like Rissoa or silt-loving species - capitellid polychaetes, oligochaetes, that like the finer sediment deposits in seagrasses. L.orbiculatus is a very characteristic seagrass-associated bivalve which has endosymbiotic sulfate-reducing bacteria contributing to sometimes ~60% of its nutrition - it therefore likes the reduced sediments in seagrass beds.  

Now try differences between sites.. 
```{r simper_bn_gr_2012_sites}
simper.bn.gr.2012.sites <- simper(zoo.abnd.2012 %>%
                                    select(-c(site:replicate)) %>%
                                    mutate_all(sqrt),
                                  group = zoo.abnd.2012 %>% pull(site))

simper.bn.gr.2012.sites
summary(simper.bn.gr.2012.sites)
```
 
Yeees more or less the same thing; as I said, the sand samples are mostly responsible for the differences, so... 

Do the within-group SIMPERs, as well.. 
```{r simper_wn_gr_2012_habitats}
simper.wn.gr.2012.habitats <- simper_within_group(zoo.abnd.2012 %>% select(-c(site:replicate)),
                                                  group = zoo.abnd.2012 %>% pull(habitat))

simper.wn.gr.2012.habitats
```

As I said before - sand samples have pretty low average group similarity, 27% (you can see that, they're dispersed around the MDS plot). This is not due to striking differences between sites, but to the taxonomic identification problems causing high variability. 
The most characteristic species for the sand habitat are: B.reticulatum, L.orbiculatus, C.gallina, C.capitata. 
The most characteristic species for the seagrass habitat are: B.reticulatum, L.orbiculatus, M.lineatus, R.membranacea. 
See, they overlap; there is no clear difference sand/seagrass here.. 

SIMPER within sites.. 
```{r simper_wn_gr_2012_sites}
simper.wn.gr.2012.sites <- simper_within_group(zoo.abnd.2012 %>% select(-c(site:replicate)),
                                               group = zoo.abnd.2012 %>% pull(site))

simper.wn.gr.2012.sites
```
For the site groupings, Konski seems to have more Oligochaetes and Capitellid and spionid polychaetes - consistent with its proximity to the disturbance source - more organic loading so a higher proportion of tolerant species. Ribka and Gradina, on the other hand, have mostly mollusks - B.reticulatum, L.orbiculatus, R.membranacea.. The within-group similarities here are better than at the habitat level (because the sand samples are so very different among themselves).  

Save all SIMPERs, no matter how flawed. 
```{r save_simper_2012}
write_rds(simper.bn.gr.2012.habitats, 
          here(save.dir, "simper_bn_group_habitats_2012.RDS"))

write_rds(simper.bn.gr.2012.sites, 
          here(save.dir, "simper_bn_group_sites_2012.RDS"))

write_rds(simper.wn.gr.2012.habitats, 
          here(save.dir, "simper_wn_group_habitats_2012.RDS"))

write_rds(simper.wn.gr.2012.sites, 
          here(save.dir, "simper_wn_group_sites_2012.RDS"))

```

Now do between-group SIMPERs using the **biomasses**.  
```{r simper_biomass_bn_gr_2012_habitats}
simper.biomass.bn.gr.2012.habitats <- simper(zoo.biomass.2012 %>%
                                               select(-c(site:replicate)) %>% 
                                               mutate_all(sqrt),
                                             group = zoo.biomass.2012 %>% pull(habitat))

simper.biomass.bn.gr.2012.habitats
summary(simper.biomass.bn.gr.2012.habitats)
```

SIMPER with biomasses between sites, this time.. 
```{r simper_biomass_bn_gr_2012_sites}
simper.biomass.bn.gr.2012.sites <- simper(zoo.biomass.2012 %>%
                                            select(-c(site:replicate)) %>%
                                            mutate_all(sqrt),
                                          group = zoo.biomass.2012 %>% pull(site))

simper.biomass.bn.gr.2012.sites
summary(simper.biomass.bn.gr.2012.sites)
```
