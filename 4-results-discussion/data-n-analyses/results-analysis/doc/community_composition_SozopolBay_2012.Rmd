...
```
  ...
  The **summarized abundance** by station and habitat is: 
...
```
  ...
  The **summarized biomass** by station and habitat is: 
...
(plot.tax.props.site.hab.2012 <- ggplot(tax.props.site.hab.2012) + 
    geom_bar(mapping = aes(x = site,
             ...
             stat = "identity", position = "stack", inherit.aes = TRUE) +
    scale_fill_viridis(option = "viridis", discrete = TRUE, name = "") +
    scale_x_discrete(labels = c("K", "R", "G")) +
    labs(x = "Site", y = "%") + 
    # change the strip labels
    facet_wrap(~habitat, nrow = 2, labeller = as_labeller(c("S" = "sand", "Z" = "Zostera"))) +
...
tax.props.2012.site.all <- full_join(tax.props.site.hab.2012, 
  ...
  ungroup() %>%
  # fix a little the variables that will serve for faceting & labeling the plot, to avoid re-labeling later 
  mutate(variable = case_when(variable == "proportion.x" ~ "Abundance", 
                              variable == "proportion.y" ~ "Biomass"), 
         habitat = case_when(habitat == "S" ~ "sand", 
                             habitat == "Z" ~ "Zostera"), 
         site = case_when(site == "Konski" ~ "K", 
                          site == "Ribka" ~ "R", 
                          site == "Gradina" ~ "G")) %>% 
  # turn site into factor to preserve order as before 
