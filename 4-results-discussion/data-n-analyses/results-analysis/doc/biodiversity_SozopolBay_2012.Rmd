...
```{r summarize_nb_taxa_2012_by_habitat_site_1}
...
```{r summarize_nb_taxa_2012_by_habitat_site_2}
specnumber(filter(zoo.abnd.2012, habitat == "S") %>% select(-c(site:replicate)), groups = filter(zoo.abnd.2012, habitat == "S")$site)
```

  The **summarized number of taxa** by site and year is: 
```{r summarize_nb_taxa_2012_by_site_year}
zoo.abnd.2012.long %>%
  group_by(habitat, site, station, replicate) %>%
  transmute(total_nb_taxa = specnumber(sp_count)) %>%
...
```
  ...
  The **summarized abundance** by site and year is: 
```{r summarize_abundance_2012_by_site_year}
## will do it interactively, because it's easier to switch the grouping factors (not so obvious with a function)
zoo.abnd.2012.long %>% 
  group_by(habitat, site, station, replicate) %>%
  transmute(total = sum(sp_count) * 1/0.0078) %>%
...
```
  ...
  The **summarized biomass** by site and year is: 
```{r summarize_biomass_2012_by_site_year}
zoo.biomass.2012.long %>% 
  group_by(habitat, site, station, replicate) %>%
  transmute(total = sum(sp_biomass) * 1/0.0078) %>%
  distinct(habitat, site, total) %>%
...
...and the **contribution of the taxonomic groups to the abundance by site & habitat**: 
...
get_top_sp <- function(zoo.data.long, species.col, value.col, nb.sp = 20, ...) {
  ...
  # ... - grouping variables for summary
  ...
  top.n.st <- zoo.data.long %>%
    filter(!!sp_col %in% top.n %>% select(!!sp_col)) %>%
    select(!!!grouping_vars, !!sp_col) 

  return(top.n.st)
}


top20.counts.2012.st <- get_top_sp(zoo.abnd.2012.long, nb.sp = 20, species.col = species, value.col = sp_count, habitat, station)
