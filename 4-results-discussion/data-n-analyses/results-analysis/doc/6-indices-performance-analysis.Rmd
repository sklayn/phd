...
plot.ind.st.summary.sand <- ggplot(ind.st.summary.sand.long, 
  ...
  geom_text(aes(x = as.numeric(station), y = mean + sd, label = label), vjust = -0.3, size = 2) +
...
plot.ind.biotope.summary.sand <- ggplot(ind.biotope.summary.sand.long, 
  ...
  geom_text(aes(x = as.numeric(biotope), y = mean + sd, label = label), vjust = -0.3, size = 2) +
...
Make a composite plot of both stations and biotopes, to save myself the trouble later.  
```{r plot_composite_ind_summary_st_biotopes_sand}
(plot.comp.ind.st.biotope.summary.sand <- cowplot::plot_grid(plot.ind.st.summary.sand,
                                                             plot.ind.biotope.summary.sand,
                                                             nrow = 2,
...
```{r save_plot_composite_ind_summary_st_biotopes_sand}
cowplot::save_plot(here(figures.dir, "ind_st_biotope_summary_sand.png"), 
                   plot.comp.ind.st.biotope.summary.sand, 
                   base_aspect_ratio = 2, base_width = 6.8, base_height = 6)
```



Better idea: instead of having the ES limits as lines, do them as strips of colour. Might be a more intuitive visualization.. Also plot the mean EQRs as dots instead of bars (bars are generally a waste of ink). 
```{r ind_limits_ranges}
## we want ranges (ymin-ymax) to plot the classes as strips of colour. The values in the index limit tibble are the lower limits of each class = ymin (except for bad, which is 0 and will have to be added). The upper limits are Inf for high, and the lower limit of the previous class for all others. Bonus: will be the same for the seagrasses, so won't have to repeat this calculation.   
(ind.limits.strips.long <- ind.limits.long %>%
   ## add a row of 0 for each index - corresponding to the lower limit for the bad class 
   bind_rows(tibble(index = c("H'", "AMBI", "M-AMBI", "BENTIX", "M-AMBIn"), 
                    limits = rep(0, times = 5))) %>% 
   arrange(index) %>%
   group_by(index) %>%
   ## calculate the ymin and ymax for each index, as described above
   mutate(ymin = limits, 
          ymax = lag(limits, 1)) %>% 
   ## set ymax for high to +Inf
   mutate(ymax = replace_na(ymax, Inf)) %>% 
   ## add class designation (as factor) - to be used in the plot legend
   mutate(class = factor(c("high", "good", "moderate", "poor", "bad"), levels = c("high", "good", "moderate", "poor", "bad")))
)

## make a new named vector of colours for the classes (yes I know it's technically the same as the EG colours, in fact I'm reusing it, but I don't want too much confusion)
es.classes <- eg.colours
names(es.classes) <- as.character(levels(ind.limits.strips.long$class))
```

On to the monstrous plot!
```{r plot_ind_summary_stations_strips_sand}
(plot.ind.st.summary.strips.sand <- ggplot(ind.st.summary.sand.long,
                                           aes(x = as.numeric(station), y = mean)) +
   ## add the strips first - so as to form the background, not overlap the points
   geom_rect(data = ind.limits.strips.long, 
             aes(xmin = -Inf, xmax = Inf, ymin = ymin, ymax = ymax, fill = class), 
             alpha = 0.45, inherit.aes = FALSE) + 
   scale_fill_manual(name = "Ecological class", values = es.classes) +
   
   ## error bars = sd. Plot first, so as to go behind the dots
   geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd),
                 width = 0.15, colour = "grey25", position = position_dodge(0.9)) +
...
Save this - wow was it a pain to construct, I really don't want to lose it somewhere... 
```{r save_plot_ind_summary_stations_strips_sand}
ggsave(here(figures.dir, "ind_station_summary_strips_sand.png"),
       plot.ind.st.summary.strips.sand, 
       dpi = 300, width = 17, height = 15, units = "cm")
```


Now plot the biotopes (keeping the x axis numeric for readability).  
```{r plot_ind_summary_biotopes_strips_sand}
(plot.ind.biotope.summary.strips.sand <- ggplot(ind.biotope.summary.sand.long, 
                                                aes(x = as.numeric(biotope), y = mean)) + 
   ## add the strips first - so as to form the background, not overlap the points
   geom_rect(data = ind.limits.strips.long, 
             aes(xmin = -Inf, xmax = Inf, ymin = ymin, ymax = ymax, fill = class), 
   ...
   ## error bars = sd. Before the points, so that the bars don't overlap them  
   geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd),
                 width = 0.15, colour = "grey25", position = position_dodge(0.9)) +
   geom_point(colour = "grey25") +
...
Save this one, too. 
```{r save_plot_ind_summary_biotopes_strips_sand}
ggsave(here(figures.dir, "ind_biotope_summary_strips_sand.png"),
       plot.ind.biotope.summary.strips.sand, 
       dpi = 300, width = 17, height = 15, units = "cm")
```



Make a composite plot of both stations and biotopes, with EC as colour strips.  
```{r plot_composite_ind_summary_st_biotopes_strips_sand}
(plot.comp.ind.st.biotope.summary.strips.sand <- cowplot::plot_grid(plot.ind.st.summary.strips.sand + theme(legend.position = "none"),
                                                                    plot.ind.biotope.summary.strips.sand + theme(legend.position = "none"),
                                                                    nrow = 2,
                                                                    align = "h", labels = c("A", "B"))
)

## get the legend from one of the plots, and put it at the bottom
comp.ind.legend <- cowplot::get_legend(plot.ind.biotope.summary.strips.sand + theme(legend.position = "right"))

(plot.comp.ind.st.biotope.summary.strips.sand <- cowplot::plot_grid(plot.comp.ind.st.biotope.summary.strips.sand,
                                                                    comp.ind.legend,
...
Save this monstrosity.  
```{r save_plot_composite_ind_summary_st_biotopes_strips_sand}
cowplot::save_plot(here(figures.dir, "ind_st_biotope_summary_strips_sand.png"), 
                   plot.comp.ind.st.biotope.summary.strips.sand, 
                   base_aspect_ratio = 2, base_width = 6.8, base_height = 7)
```
...
**NOTE 2019/05/07: I checked with the published article on this - there are some discrepancies in the correlation coefficients and significance levels (and also with the environmental parameters used). I suppose it's because for the thesis, I redid most of the calculations, and notably imputed (mean value) the missing values for the sediment parameters - there is so little data there anyway.. I don't know what precisely is going on with the NO3 for instance. For practical purposes, I am going with the published results in the article - NOT the ones obtained here; and with the parameters there, too. In any case, they aren't too different - the conclusions are exactly the same - and it will be too much hassle to explain the mismatch.**  

Last-last thing to try: given the environmental parameters, check if the classification system successfully differentiates between between classes, and between which subsequent classes - best?   
I'm only going to use the environmental parameters with which the indices have the best correlations (yes, I know it is highly subjective, and I don't care). Also, I'm importing and using the **old environmental data from the article**. 
Import the old environmental data (pre-cleaned and everything).  
```{r import_old_env_data_sand}
(env.data.old.sand <- read_csv(here(data.dir, "env_data_smry_old_sand.csv"))
)
```

Gather the needed data (yes, I'm making another two-three-million useless temporary data frames).  
```{r class_distr_ind_env_sand}
## get the index EQRs, and convert to qualitative. MAKE EXTRA SURE THE COLUMNS ARE IN THE SAME ORDER IN THE TWO DFS, OTHERWISE THE WRONG LIMITS WILL BE APPLIED TO EACH INDEX
(ind.qual.sand <- map2_df(indices.all.norm.sand %>% select(H.EQR, AMBI.EQR, `M-AMBI`, BENTIX.EQR, `M-AMBIn.EQR`), 
                             ind.limits,
                             ~es_qualitative(.x, .y)
                             )
)
...
## get (manually) the environmental parameters, and add to the qualitative index df. AGAIN MAKE SURE THE ROWS ARE IN THE SAME ORDER (I TRUST THEY ARE HERE, BUT IN FUTURE)
(ind.qual.env.sand <- bind_cols(ind.qual.sand, 
                                   env.data.old.sand %>% select(silt.clay, Secchi.depth_mean, chl.a_mean, NH4_mean, PO4_mean, LUSI))
)  

## convert to long, key = index
(ind.qual.env.long.sand <- gather(ind.qual.env.sand, key = "index", value = "assessment", -c(silt.clay:LUSI)) %>% 
    ## convert the qualitative assessments to factor, so that the classes go high to bad
    mutate(assessment = factor(assessment, levels = c("high", "good", "moderate", "poor", "bad")))
)
```

Make a boxplot of the distribution of classes vs the environmental parameters for each index.  
```{r plot_class_distr_ind_env_sand}
## to avoid repeating code for each variable, AND to have nice y-axis labels.. 
map2(ind.qual.env.long.sand %>% select(silt.clay:LUSI),
     names(ind.qual.env.long.sand %>% select(silt.clay:LUSI)),
     ~ggplot(ind.qual.env.long.sand, aes(x = assessment, y = .x)) +
       geom_boxplot() +
       labs(y = .y) +
       facet_wrap(~index))
...
I don't think I'm using this, in the end.. That's why I'm not going to try and make the plots more presentable.  
As expected from the correlations - for some index-parameter combinations, not bad at all - classification distinct enough, and in the correct direction to boot. See AMBI (and BENTIX, though a bit less) with silt.clay, chl.a, secchi, LUSI. 
The other three are shitty, though - H' and M-AMBI for the reasons already explained, M-AMBIn because not adjusted for most of the habitat subtypes in the dataset.  
Funny thing - no bad anywhere (some in H, which as established performs really poorly in some particular conditions which I was unlucky enough to get in my data).  
...
(plot.ind.abs.av.class.diff.sand <- ggplot(ind.abs.av.class.diff.sand.long, 
    ...
    geom_hline(yintercept = 0.5, col = "red") + # the acceptable difference limit according to the intercalibration
    coord_flip() + 
...
Check in how many cases the diffences cross the good-moderate border - that would be the one requiring measures to be taken.    
```{r index_diff_good_moderate_sand}
## custom function, and a pain in the ass it was to write. Also probably could have been done much more easily and elegantly. 
gm.diff.sand <- gm_diff(indices.all.norm.sand, ind.limits)

## fix index names, because I have OCD
(gm.diff.sand <- gm.diff.sand %>% 
    mutate(contrast = str_replace_all(contrast, pattern = ".EQR", replacement = ""))
)
```

In most cases, the difference in assessment between two indices crosses the good/moderate boundary, so would have implications - for both conservation and management. On one hand, if the actual ES is acceptable, and the assessment says the opposite, unnecessary measures and expenses would be undertaken. On the other hand, if the reverse happens, no measures would be undertaken, and the ES is likely to deteriorate even more. Of course, it's probably not likely that a community in terrible, horrible, no good condition will ever be assessed as high, with no one the wiser - but it is what it is.   
Surprisingly good result - only 5% - for M-AMBI/M-AMBIn. It gives one hope for the comparability of results of the old and new national monitoring program, and for the validity of management measures (or it would, if anyone else involved were doing their jobs).  
It seems best to use more than one index, as suggested by numerous other studies before (which??) - and make sure they are more or less in agreement, and react as expected to the pressure that is currently investigated. Also, if one index is a component in the calculation of another (see: H and M-AMBI - both flavours) - make sure it doesn't have an unduly high weight, esp. if its behaviour is sometimes non-linear or dependent on hidden factors (or take those into consideration as well).  
...
ind.st.labels.zostera.long <- ind.st.labels.zostera %>%
    gather(key = station, value = label, -.id) %>%
    rename(index = .id) %>%
    mutate(station = factor(station, levels = levels(indices.all.zostera$station)))
...
Redo this plot with EC as strips of colour in the background.  
```{r plot_ind_summary_stations_strips_zostera}
(plot.ind.st.summary.strips.zostera <- ggplot(ind.st.summary.zostera.long,
                                              aes(x = as.numeric(station), y = mean)) +
   ## add the strips first - so as to form the background, not overlap the points
   geom_rect(data = ind.limits.strips.long, 
             aes(xmin = -Inf, xmax = Inf, ymin = ymin, ymax = ymax, fill = class), 
   ...
   geom_point(colour = "grey25") +
   ## labels - significant difference pairs (above error bars)
   geom_text(aes(x = as.numeric(station), y = mean + sd, label = label), vjust = -0.5, size = 2) +
   labs(x = "Station", y = "EQR") +
   facet_wrap(~index) +
   scale_y_continuous(breaks = seq(0, 1, 0.25)) +
   scale_x_continuous(breaks = 1:5, labels = paste0("Z", unique(as.numeric(ind.st.summary.zostera.long$station))))
...
Save this. 
```{r save_plot_ind_summary_stations_strips_zostera}
ggsave(here(figures.dir, "ind_station_summary_strips_zostera.png"),
       plot.ind.st.summary.strips.zostera, 
       dpi = 300, width = 17, height = 15, units = "cm")
```
...
Last-last thing to try: given the environmental parameters, check if the classification system successfully differentiates between between classes, and between which subsequent classes - best?   
I'm only going to use the environmental parameters with which the indices have the best correlations (yes, I know it is highly subjective, and I don't care).  
Gather the needed data (yes, I'm making another two-three-million useless temporary data frames).  
```{r class_distr_ind_env_zostera}
## get the index EQRs, and convert to qualitative. MAKE EXTRA SURE THE COLUMNS ARE IN THE SAME ORDER IN THE TWO DFS, OTHERWISE THE WRONG LIMITS WILL BE APPLIED TO EACH INDEX
(ind.qual.zostera <- map2_df(indices.all.norm.zostera %>% select(H.EQR, AMBI.EQR, `M-AMBI`, BENTIX.EQR, `M-AMBIn`), 
                             ind.limits,
                             ~es_qualitative(.x, .y)
                             )
)
...
## get (manually) the environmental parameters, and add to the qualitative index df. AGAIN MAKE SURE THE ROWS ARE IN THE SAME ORDER (I TRUST THEY ARE HERE, BUT IN FUTURE)
(ind.qual.env.zostera <- bind_cols(ind.qual.zostera, 
                                   env.data.zostera %>% select(TOM, chl_a, secchi, LUSI, shoot_count))
)  

## convert to long, key = index
(ind.qual.env.long.zostera <- gather(ind.qual.env.zostera, key = "index", value = "assessment", -c(TOM:shoot_count)) %>% 
    ## convert the qualitative assessments to factor, so that the classes go high to bad
    mutate(assessment = factor(assessment, levels = c("high", "good", "moderate", "poor", "bad")))
)
```
...
Make a boxplot of the distribution of classes vs the environmental parameters for each index.  
```{r plot_class_distr_ind_env_zostera}
## to avoid repeating code for each variable, AND to have nice y-axis labels.. 
map2(ind.qual.env.long.zostera %>% select(TOM:shoot_count),
     names(ind.qual.env.long.zostera %>% select(TOM:shoot_count)),
     ~ggplot(ind.qual.env.long.zostera, aes(x = assessment, y = .x)) +
       geom_boxplot() +
       labs(y = .y) +
       facet_wrap(~index))
...
I don't think I'm using this, in the end.. That's why I'm not going to try and make the plots more presentable.  
As expected from the correlations - there is much variability in the index assessments, so the classes tend to overlap (no clear distinction often enough). Nevertheless, you can see that the assessments mostly go in the correct direction, and the indices do exhibit classification differences corresponding to increases/decreases of the environmental parameters (which should have been tested statstically, say, with a t-test on subsequent classes, 2x2, but since I'm not going to use it, I won't bother).  
Exception - H - a total mess; it wasn't correlated to anything, so no surprise there.  
...
ind.percent.agreement.zostera <- as_tibble(ind.abs.class.diff.zostera) %>% 
    gather(key = ind, value = diff) %>%
    ## recoding the class difference so that all differences of > 2 classes form a single group not necessary here - there are no > 2 classes differences
    # mutate(diff = recode(diff, "0" = 0, "1" = 1, "2" = 2, "3" = 3, "4" = 3)) %>%
    group_by(ind, diff) %>%
    summarize(count = n()) %>%
    ungroup()
...
Surprising: difference M-AMBI-M-AMBIn - total disagreement. Those old boundaries must really be unsuitable for the seagrass communities..   
...
Check in how many cases the diffences cross the good-moderate border - that would be the one requiring measures to be taken.    
```{r index_diff_good_moderate_zostera}
## custom function
gm.diff.zostera <- gm_diff(indices.all.norm.zostera, ind.limits)

## fix index names, because I have OCD
(gm.diff.zostera <- gm.diff.zostera %>% 
    mutate(contrast = str_replace_all(contrast, pattern = ".EQR", replacement = ""))
)
```

NB this is different than the differences between stations (see plot), because the **normalized EQRs** are used. If the non-normalized (really observed) EQRs are used, the proportion of differences that cross the G/M boundary are smaller. 

Well this was an exercise in futility.  
The most surprising result again is M-AMBI-M-AMBIn - 100% of the cases are on opposite sides of the GM boundary. This means that one really, really shouldn't use M-AMBIn and the old boundaries if one wants something resembling a sensible result.  
The others are more or less as expected, given the previous agreement analyses. The best is AMBI-BENTIX - it happens in only 12% of the cases.  

All in all, the soft-bottom zoobenthic indices as they are applied here are really bad in the seagrass. Possible solutions: 1) adapt the boundaries to seagrass-associated fauna - good luck, with the amount of variation observed, and the interference of the natural environmental particularities, see community structure analyses 2) make/adapt a new index, specifically for seagrass fauna 3) drop entirely from assessments - no point; probably sufficient to monitor ecological state of seagrass itself - if it's good, it's likely that the associated communities will do perfectly fine. Bonus: those assessments are immensely easier to apply/train people to do.    
