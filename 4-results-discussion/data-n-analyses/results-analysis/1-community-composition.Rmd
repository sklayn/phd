...
plot_tax_props <- function(tax.props.data) {
  
  # station needs to be converted to numeric, because otherwise the labels overlap
  nb.stations <- length(unique(tax.props.data$station))
  
  ggplot(tax.props.data) + 
    geom_bar(mapping = aes(x = as.numeric(station),
                           y = proportion * 100, 
                           fill = group), 
             stat = "identity", position = "stack", inherit.aes = TRUE) +
    scale_fill_viridis(discrete = TRUE, name = "") +
    scale_x_continuous(breaks = c(1:nb.stations)) + 
    labs(x = "Station", y = "%") + 
    facet_wrap(~year, nrow = 2) + 
    theme_bw()
}

(plot.tax.props.st.yr.sand <- plot_tax_props(tax.props.st.yr.sand) +
...
# plot this
(plot.tax.props.biomass.st.yr.sand <- plot_tax_props(tax.props.biomass.st.yr.sand) + 
   text_size(text.x = rel(1.3), text.y = rel(1.3), title.x = rel(1.3), legend.text = rel(1.3), strip.x = rel(1.3))
)
...
write_csv(zoo.abnd.zostera, 
          path = file.path(save.dir, "abnd_zostera_orig_clean.csv"))
write_csv(zoo.biomass.zostera, 
          path = file.path(save.dir, "biomass_zostera_orig_clean.csv"))
```
...
ggsave(file.path(figures.dir, "nb_taxa_by_tax_group_zostera.png"), 
       plot.nb.taxa.by.group, 
       dpi = 300)
```
...
(plot.tax.props.st.yr.zostera <- plot_tax_props(tax.props.st.yr.zostera) +
    text_size(text.x = rel(1.3), text.y = rel(1.3), title.x = rel(1.3), legend.text = rel(1.3), strip.x = rel(1.3))
)

## save as file
ggsave(file.path(figures.dir, "tax_gr_props_st_yr_zostera.png"), 
...
plot.most.abnd.sp.zostera <- plot_top_n(top20.counts.zostera.st, 
                                     aes(x = species, y = log_y_min(sp_count), 
                                         colour = station), 
                                     labs.legend = as.numeric(unique(top20.counts.zostera.st$station)), 
                                     lab.y = "Abundance (log(y/min + 1))", 
                                     palette = "Set2"
                                     )
...
## save as file
ggsave(file.path(figures.dir, "most_abnd_sp_st_zostera.png"), 
...
# plot this
(plot.tax.props.biomass.st.yr.zostera <- plot_tax_props(tax.props.biomass.st.yr.zostera) + 
   text_size(text.x = rel(1.3), text.y = rel(1.3), title.x = rel(1.3), legend.text = rel(1.3), strip.x = rel(1.3))
)

## save as file
ggsave(file.path(figures.dir, "tax_gr_props_biomass_st_yr_zostera.png"), 
...
plot.highest.biomass.sp.zostera <- plot_top_n(top20.biomass.zostera.st, 
                                     aes(x = species, y = log_y_min(sp_count), 
                                         colour = station), 
                                     labs.legend = as.numeric(unique(top20.biomass.zostera.st$station)), 
                                     lab.y = "Biomass (log(y/min + 1))",
                                     palette = "Set2"
                                     )
...
## save as file
ggsave(file.path(figures.dir, "highest_biomass_sp_st_zostera.png"), 
...
#### Macrozoobenthic communities in the seagrass and sand bottoms of Sozopol Bay (2012)  
  And again, for the seagrass/sand communities in Sozopol Bay in 2012 (preliminary experiment).  
  Import the zoobenthic abundance & biomass data. 
...
write_csv(zoo.abnd.2012, 
          path = file.path(save.dir, "abnd_2012_orig_clean.csv"))
write_csv(zoo.biomass.2012, 
          path = file.path(save.dir, "biomass_2012_orig_clean.csv"))
```
...
taxa.sand.2012 <- unique(zoo.abnd.2012 %>% 
  ...
  select(species))
```
  There are `r nrow(semi_join(taxa.sand.2012, taxa.zostera.2012))` taxa found in both sand and seagrass in 2012. Let's see them:
```{r common_taxa_sand_zostera_2}
semi_join(taxa.sand.2012, taxa.zostera.2012)
```
  The taxa found **only in sand** are:
```{r taxa_sand_only_2012}
anti_join(taxa.sand.2012, taxa.zostera.2012)
```
  The taxa found **only in seagrass** are: 
```{r taxa_zostera_only_2012}
anti_join(taxa.zostera.2012, taxa.sand.2012)
```
...
## in seagrass
summarize_zoo_data(filter(zoo.abnd.2012.long, habitat == "Z"), value.col = sp_count, per.m2 = 1/0.0078, round.to = 0)

## in sand
summarize_zoo_data(filter(zoo.abnd.2012.long, habitat == "S"), value.col = sp_count, per.m2 = 1/0.0078, round.to = 0)
```
  
  The **summarized biomass** by station and year is: 
```{r summarize_biomass_2012_by_station_year}
## in seagrass
summarize_zoo_data(filter(zoo.biomass.2012.long, habitat == "Z"), value.col = sp_biomass, per.m2 = 1/0.0078, round.to = 2)

## in sand 
summarize_zoo_data(filter(zoo.biomass.2012.long, habitat == "S"), value.col = sp_biomass, per.m2 = 1/0.0078, round.to = 2)
```
<br>  
  2. Taxonomic composition and structure  
...
# in seagrass
left_join(taxa.zostera.2012, current.taxa.2012) %>%
  count(class, sort = TRUE) %>%
  ggplot(mapping = aes(x = reorder(class, n), y = n)) + 
    geom_bar(stat = "identity", fill = "light green") + 
    coord_flip() + 
...
# in sand
left_join(taxa.sand.2012, current.taxa.2012) %>%
  count(class, sort = TRUE) %>%
  ggplot(mapping = aes(x = reorder(class, n), y = n)) + 
...
# in seagrass
plot.nb.taxa.by.group.Z.2012 <- left_join(taxa.zostera.2012, current.taxa.2012) %>% 
  ungroup %>%
  count(group) %>%
  ggplot(mapping = aes(x = reorder(group, n), y = n)) + 
    geom_bar(stat = "identity", fill = "light green") + 
...
# in sand 
plot.nb.taxa.by.group.S.2012 <- left_join(taxa.sand.2012, current.taxa.2012) %>% 
  ungroup %>%
  count(group) %>%
  ggplot(mapping = aes(x = reorder(group, n), y = n)) + 
    geom_bar(stat = "identity", fill = "light blue") + 
...
## great time to try out package cowplot & arrange these two plots on one grid, horizontally
library(cowplot)

plot_grid(plot.nb.taxa.by.group.Z.2012, 
          plot.nb.taxa.by.group.S.2012, 
          labels = c("A", "B"), 
          align = "h")

# of course, if I end up doing this, both axes should have the same limits - otherwise too difficult to compare & potentially misleading. 

# ggsave(file.path(figures.dir, "nb_taxa_by_tax_group_2012.png"), 
#        plot.nb.taxa.by.group, 
#        dpi = 300)
```
