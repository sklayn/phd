...
plot_tax_props <- function(tax.props.data, x.labels = NULL) {
...
(plot.tax.props.st.yr.sand <- plot_tax_props(tax.props.st.yr.sand, x.labels = "S") +
...
# plot this
(plot.tax.props.biomass.st.yr.sand <- plot_tax_props(tax.props.biomass.st.yr.sand, x.labels = "S") + 
    facet_wrap(~year, nrow = 2) + 
    text_size(text.x = rel(1.3), text.y = rel(1.3), title.x = rel(1.3), legend.text = rel(1.3), strip.x = rel(1.3))
)
...
```
  ...
  In any case, the 2012 dataset should be regarded as very preliminary, with a really low confidence level, and all conclusions based on it should only be tentative. It will not be included in the other seagrass analyses despite the similar samping strategy and methods, because due to the low species richness it biases the whole process (e.g. it might be concluded that seagrasses in Sozopol Bay were in a worse ecological state in 2012, which then improved markedly in 2013-2014, but this is not the case - the real variable that improved in that time was my skills at sorting and knowledge of taxonomy).   
...
(plot.tax.props.st.yr.2012 <- plot_tax_props(tax.props.st.yr.2012, x.labels = c("K1", "K2", "R1", "R2", "G1", "G2")) + 
...
(plot.most.abnd.sp.2012 <- plot.most.abnd.sp.2012  + 
    ...
    text_size(text.x = rel(1.3), text.y = rel(1.3), legend.text = rel(1.2), legend.title = rel(1.2), strip.x = rel(1.3)) + 
    scale_x_discrete(name = "", limits = rev(levels(top20.counts.2012.st$species))) 
)

## save as file
ggsave(file.path(figures.dir, "most_abnd_sp_st_2012.png"), 
...
## save as file
ggsave(file.path(figures.dir, "tax_gr_props_biomass_st_yr_2012.png"), 
...
## save as file
ggsave(file.path(figures.dir, "highest_biomass_sp_st_2012.png"), 
...
```
  ...
  Add the site to the (long) abundance & biomass datasets.  
```{r add_site_2012}
