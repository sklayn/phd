...
setdiff(taxa.sand.2012, taxa.zostera.2012)
```
  The taxa found **only in seagrass** are: 
```{r taxa_zostera_only_2012}
setdiff(taxa.zostera.2012, taxa.sand.2012)
```
...
### TO FIX THIS CRAP - allow faceting by habitat/selectable var?? 
tax.props.st.yr.2012 <- lapply(c("Z", "S"), function(x) {
  zoo.abnd.2012.long %>% 
  filter(habitat == x) %>%
  summarize_tax_group_contr(current.taxa.2012, 
                            key.col = "species", 
...
(plot.tax.props.st.yr.2012 <- lapply(tax.props.st.yr.2012, 
                                     function(x) {
                                      p <- plot_tax_props(x) + 
                                        text_size(text.x = rel(1.3), 
                                                  text.y = rel(1.3), 
                                                  title.x = rel(1.3), 
                                                  legend.text = rel(1.3), 
                                                  strip.x = rel(1.3)) + 
                                        scale_x_discrete(breaks = c("K1", "K2", "R1", "R2", "G1", "G2"))
                                      return(p)
                                     })
)
...
## save as file
# ggsave(file.path(figures.dir, "tax_gr_props_st_yr_2012.png"), 
